    ...
    state.items.forEach((item, idx) => {
      ...
      div.innerHTML = `
        ...
        <div class="flex flex-nowrap gap-3 overflow-x-auto pb-2">
          <div class="min-w-[180px] flex-[2]"><label class="block text-sm text-slate-500 mb-1 whitespace-nowrap">${t('common.quantity')}</label><input type="number" min="0" step="1" data-field="quantity" data-id="${item.id}" value="${item.quantity}" class="form-input h-10 text-base py-2 w-full"></div>
          ...
          <div class="min-w-[200px] flex-[2]"><label class="block text-sm text-slate-500 mb-1 whitespace-nowrap">${t('common.unitPrice')}</label><input type="number" min="0" step="0.01" data-field="unitPrice" data-id="${item.id}" value="${item.unitPrice}" class="form-input h-10 text-base py-2 w-full"></div>
