// Industries shared JS — nav, footer, language toggle
...
const FOOTER_LABELS = {
  en: { industries: 'Industries', moreIndustries: 'More Industries', company: 'Company', tools: 'Tools', legal: 'Legal' },
  ar: { industries: 'القطاعات', moreIndustries: 'قطاعات أخرى', company: 'الشركة', tools: 'الأدوات', legal: 'قانوني' }
};
...
const PCIndustries = {
        ...
        <div class="flex items-center justify-between h-16">
          ...
          <div class="flex items-center gap-2">
            ...
            <div class="flex items-center gap-1 bg-slate-100 rounded-lg p-1">
              <button data-ind-lang="en" class="ind-lang-btn px-3 py-1.5 text-xs font-semibold rounded-md ${this.lang === 'en' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600'}">EN</button>
              <button data-ind-lang="ar" class="ind-lang-btn px-3 py-1.5 text-xs font-semibold rounded-md ${this.lang === 'ar' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-600'}">AR</button>
            </div>
          </div>
        </div>
      </div>
    `;
    document.querySelectorAll('.ind-lang-btn').forEach(btn => {
  ...
  renderFooter() {
    const footer = document.getElementById('ind-footer');
    if (!footer) return;
    const isAr = this.lang === 'ar';
    const indLinks = [
      { href: './construction.html', en: 'Construction', ar: 'الإنشاء' },
      { href: './restaurants.html', en: 'Restaurants', ar: 'المطاعم' },
      { href: './clinics.html', en: 'Clinics', ar: 'العيادات' },
      { href: './schools.html', en: 'Schools', ar: 'المدارس' },
      { href: './ngos.html', en: 'NGOs', ar: 'المنظمات غير الربحية' },
      { href: './retail.html', en: 'Retail', ar: 'التجزئة' },
      { href: './manufacturing.html', en: 'Manufacturing', ar: 'التصنيع' },
      { href: './transportation.html', en: 'Transportation', ar: 'النقل' }
    ];
    const companyLinks = [
    ...
    footer.innerHTML = `
      <div class="max-w-6xl mx-auto px-4 py-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-8 text-sm text-slate-500">
        <div>
          <h3 class="font-semibold text-slate-700 mb-3">${this.FL('industries')}</h3>
          <ul class="space-y-1">${renderLinks(indLinks.slice(0, 4))}</ul>
        </div>
        <div>
          <h3 class="font-semibold text-slate-700 mb-3">${this.FL('moreIndustries')}</h3>
          <ul class="space-y-1">${renderLinks(indLinks.slice(4))}</ul>
        </div>
        <div>
  ...
  applyLang() {
    ...
    document.querySelectorAll('[data-i18n-en]').forEach(el => {
      if (this.lang === 'ar') {
        ...
        if (val && /[\u2500-\u257F]/.test(val)) {
          // Mojibake detected — use English as fallback since generator fixes this
          val = el.dataset.i18nEn;
        }
...
document.addEventListener('DOMContentLoaded', () => PCIndustries.init());
