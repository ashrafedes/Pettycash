...
function renderNavbar() {
  ...
  const langOptions = LANGS.map(l =>
    <button data-lang='' class='w-full text-start px-3 py-2 text-sm hover:bg-slate-100 rounded-md '></button>
  ).join('');
  ...
  const linksHtml = navLinks.map(l => {
    ...
    return <a href='' class=' px-3 py-2 rounded-lg text-sm font-medium transition-colors'></a>;
  }).join('');

  const mobileLinksHtml = navLinks.map(l => {
    const active = (l.exact && isHome) || (!l.exact && currentPath.includes(l.href.replace('./','').replace('.html','')));
    const activeClass = active ? 'text-blue-600 bg-blue-50' : 'text-slate-700 hover:bg-slate-50';
    return <a href='' class=' block px-3 py-2.5 rounded-lg text-sm font-medium'></a>;
  }).join('');
    ...
    <header class='sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-100 shadow-sm'>
      <div class='max-w-6xl mx-auto px-4 sm:px-6'>
        <div class='flex items-center justify-between h-16'>
          ...
          <div class='hidden md:flex items-center gap-2'>
            <a href='/login' class='px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 rounded-lg transition-colors'></a>
            <a href='/register' class='px-4 py-2 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors'></a>
            <div class='relative ml-2' id='lang-dropdown'>
              ...
              <div id='lang-menu' class='hidden absolute end-0 mt-1 w-32 bg-white border border-slate-200 rounded-lg shadow-lg p-1 z-50'></div>
            </div>
          </div>
      ...
      <div id='mobile-menu' class='hidden md:hidden border-t border-slate-100 bg-white px-4 py-4 space-y-1'>
        ...
        <div class='pt-3 border-t border-slate-100 flex flex-col gap-2'>
          <a href='/login' class='block text-center px-4 py-2.5 text-sm font-medium text-slate-700 border border-slate-200 rounded-lg'></a>
          <a href='/register' class='block text-center px-4 py-2.5 text-sm font-semibold text-white bg-blue-600 rounded-lg'></a>
        </div>
      </div>
    </header>
  ;
    ...
    <footer class='bg-slate-900 text-slate-300 mt-auto'>
      <div class='max-w-6xl mx-auto px-4 sm:px-6 py-12'>
        <div class='grid grid-cols-1 sm:grid-cols-3 gap-8'>
          ...
          <div>
            <h4 class='text-white font-semibold mb-3 text-sm'></h4>
            <ul class='space-y-2 text-sm'>
              <li><a href='./' class='hover:text-white transition-colors'></a></li>
              <li><a href='./features.html' class='hover:text-white transition-colors'></a></li>
              <li><a href='./pricing.html' class='hover:text-white transition-colors'></a></li>
              <li><a href='./blog.html' class='hover:text-white transition-colors'></a></li>
              <li><a href='./about.html' class='hover:text-white transition-colors'></a></li>
              <li><a href='./contact.html' class='hover:text-white transition-colors'></a></li>
            </ul>
          </div>
          <div>
            <h4 class='text-white font-semibold mb-3 text-sm'></h4>
