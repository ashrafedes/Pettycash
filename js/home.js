...
function renderStats() {
  ...
  grid.innerHTML = data.items.map(item => `
    <div class="text-center">
      <div class="text-3xl sm:text-4xl font-extrabold text-blue-600">${item.value}</div>
      <div class="text-sm text-slate-500 mt-1">${item.label}</div>
    </div>
  `).join("");
}
...
function renderWhyUs() {
  ...
  if (!section) return;
  const data = t("whyUs");
  section.querySelector("[data-i18n='whyUs.title']").textContent = data.title;
  section.querySelector("[data-i18n='whyUs.subtitle']").textContent = data.subtitle;
  const icons = {
    ShieldCheck: `<path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path><polyline points="9 12 11 14 15 10"></polyline>`,
  ...
  section.querySelector("#why-us-grid").innerHTML = data.items.map(f => `
    <div class="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-blue-200 transition-all">
      <div class="w-12 h-12 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-xl flex items-center justify-center mb-4 shadow-md"><svg class="text-white" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${icons[f.icon] || ""}</svg></div>
...
function renderComparison() {
  ...
  section.querySelector("[data-i18n='comparison.title']").textContent = data.title;
  section.querySelector("[data-i18n='comparison.subtitle']").textContent = data.subtitle;
  const grid = section.querySelector("#comparison-grid");
  const excelCard = `
    <div class="bg-white border-2 border-red-100 rounded-2xl p-6">
      <div class="flex items-center gap-3 mb-5">
        <div class="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center"><svg class="text-red-500" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg></div>
        <h3 class="font-bold text-lg text-slate-900">${data.excel.title}</h3>
      </div>
      <ul class="space-y-3">
        ${data.excel.items.map(item => `<li class="flex items-start gap-2 text-sm text-slate-600"><svg class="text-red-400 shrink-0 mt-0.5" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="15" y1="9" x2="9" y2="15"></line><line x1="9" y1="9" x2="15" y2="15"></line></svg>${item}</li>`).join("")}
      </ul>
    </div>
  `;
  const pcCard = `
    <div class="bg-white border-2 border-green-100 rounded-2xl p-6 shadow-lg">
      <div class="flex items-center gap-3 mb-5">
        <div class="w-10 h-10 bg-green-100 rounded-xl flex items-center justify-center"><svg class="text-green-600" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></div>
        <h3 class="font-bold text-lg text-slate-900">${data.pettycash.title}</h3>
      </div>
      <ul class="space-y-3">
        ${data.pettycash.items.map(item => `<li class="flex items-start gap-2 text-sm text-slate-700"><svg class="text-green-500 shrink-0 mt-0.5" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>${item}</li>`).join("")}
      </ul>
    </div>
  `;
  grid.innerHTML = excelCard + pcCard;
}

function renderToolsShowcase() {
  const section = document.getElementById("tools-showcase-section");
  if (!section) return;
  const data = t("toolsShowcase");
  section.querySelector("[data-i18n='toolsShowcase.title']").textContent = data.title;
  section.querySelector("[data-i18n='toolsShowcase.subtitle']").textContent = data.subtitle;
  const icons = {
    ...
    FileStack: `<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline>`,
  ...
  section.querySelector("#tools-showcase-grid").innerHTML = data.items.map(item => `
    <a href="${item.url}" class="group block bg-gradient-to-br from-slate-50 to-blue-50 border border-slate-200 rounded-2xl p-6 hover:shadow-lg hover:border-blue-300 transition-all">
      <div class="w-12 h-12 bg-white rounded-xl flex items-center justify-center mb-4 shadow-sm group-hover:scale-110 transition-transform"><svg class="text-blue-600" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${icons[item.icon] || ""}</svg></div>
